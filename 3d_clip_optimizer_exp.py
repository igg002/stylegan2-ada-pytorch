...
def project(
    ...
    *,
    ...
    batch_size = 8
    dist = 2.5
    raster_image_size = 224
    sigma = 0 # 1e-7
    faces_per_pixel = 20

    # Initialize cameras.
    elev = torch.linspace(30, 150, batch_size)
    azim = torch.linspace(-180, 180, batch_size)
    R, T = look_at_view_transform(dist=dist, elev=elev, azim=azim)
    cameras = FoVPerspectiveCameras(device=device, R=R, T=T, fov=60.0, znear=0.1)
    ...
    # Load base model.
    grid_path = 'grid.obj' # TEMP
    verts, faces, aux = load_obj(grid_path)
    verts_uvs = aux.verts_uvs[None, ...].to(device)  # (1, V, 2)
    faces_uvs = faces.textures_idx[None, ...].to(device)  # (1, F, 3)
    texture_image = torch.full([1, 513, 513, 3], 1.0, device=device)
    base_mesh = Meshes(verts=[verts], faces=[faces.verts_idx]).to(device)
    base_mesh.textures = TexturesUV(verts_uvs=verts_uvs, faces_uvs=faces_uvs, maps=texture_image)

    height_multiplier_initial = 0.5
    height_multiplier = torch.tensor(height_multiplier_initial, requires_grad=True, device=device)
    ...
    optimizer = torch.optim.Adam([w_opt, height_multiplier] + list(noise_bufs.values()), betas=(0.9, 0.999), lr=initial_learning_rate)
    ...
    for step in range(num_steps):
        ...
        G_outputs = G.synthesis(ws, noise_mode='const')

        # Create a terrain mesh with the generated heightmap applied to.
        offset = (G_outputs + 1) / 2
        offset = F.interpolate(offset, (513, 513), mode='area') * height_multiplier
        offset = offset.permute(0, 2, 3, 1)[0].flatten().unsqueeze(-1)
        offset = F.pad(input=offset, pad=(0, 2), mode='constant', value=0)
        offset = offset[:, [1, 0, 2]]

        mesh = base_mesh.offset_verts(offset)
        mesh = mesh.extend(batch_size)

        # Render the terrain mesh.
        rendered = renderer(mesh, materials=materials)
        synth_images = rendered[:, :, :, :3].permute(0, 3, 1, 2)

        render_frames.append(rendered) # TEMP

        synth_images = (synth_images - image_mean[None, :, None, None]) / image_std[None, :, None, None]
        ...
        loss.backward()
        optimizer.step()
        logprint(f'step {step+1:>4d}/{num_steps}: loss {float(loss):<5.2f} dist {dist:<4.2f} c_loss_text {cosine_sim_loss_text:<4.2f} s_loss_text {spherical_loss_text:<4.2f} reg_loss {reg_loss * regularize_noise_weight:<4.2f} height_multiplier {height_multiplier:<4.2f}')
...
def run_projection(
    ...
    projected_w_steps, render_frames = project(
        G,
    ...
    if save_video:
        ...
        for frame in render_frames:
            image = (frame[0, :, :, :3] * 255).to(torch.uint8).detach().cpu().numpy()
            if target_fname:
                video.append_data(np.concatenate([target_uint8, image], axis=1))
            else:
